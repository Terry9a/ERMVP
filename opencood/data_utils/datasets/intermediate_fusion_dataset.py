...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, (cav_id, selected_cav_base) in enumerate(base_data_dict.items()):
            ...
            if  selected_cav_base['ego']:
                # print("相等")               
                ego_object_bbx_center = selected_cav_processed['object_bbx_center']
                ego_object_ids = selected_cav_processed['object_ids']

                ego_object_stack.append(ego_object_bbx_center)
                ego_object_id_stack += ego_object_ids

                        # exclude all repetitive objects
                ego_unique_indices = \
                [ego_object_id_stack.index(x) for x in set(ego_object_id_stack)]
                ego_object_stack = np.vstack(ego_object_stack)
                ego_object_stack = ego_object_stack[ego_unique_indices]

                # make sure bounding boxes across all frames have the same number
                ego_object_bbx_center = \
                np.zeros((self.params['postprocess']['max_num'], 7))
                ego_mask = np.zeros(self.params['postprocess']['max_num'])
                ego_object_bbx_center[:ego_object_stack.shape[0], :] = ego_object_stack
                ego_mask[:ego_object_stack.shape[0]] = 1
        ...
        processed_data_dict['ego'].update(
            {'object_bbx_center': object_bbx_center,
             ...
             'ego_object_bbx_center': ego_object_bbx_center,
             'ego_object_bbx_mask': ego_mask,
             'ego_object_ids': ego_object_ids,
             'ego_label_dict': ego_label_dict,
             'selected_cav_id': selected_cav_id,
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            ego_object_bbx_center.append(ego_dict['ego_object_bbx_center'])
            ego_object_bbx_mask.append(ego_dict['ego_object_bbx_mask'])
            ego_object_ids.append(ego_dict['ego_object_ids'])
            ego_label_dict_list.append(ego_dict['ego_label_dict'])

            processed_lidar_list.append(ego_dict['processed_lidar'])
            record_len.append(ego_dict['cav_num'])
            selected_cav_id_list.append(ego_dict['selected_cav_id'])
        ...
        ego_object_bbx_center = torch.from_numpy(np.array(ego_object_bbx_center))
        ego_object_bbx_mask = torch.from_numpy(np.array(ego_object_bbx_mask))
        ...
        selected_cav_id_list = torch.from_numpy(np.array(selected_cav_id_list, dtype=int))
        label_torch_dict = \
            self.post_processor.collate_batch(label_dict_list)
        
        ego_label_torch_dict = \
            self.post_processor.collate_batch(ego_label_dict_list)
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                   ...
                                   'ego_object_bbx_center': ego_object_bbx_center,
                                   'ego_object_bbx_mask': ego_object_bbx_mask,
                                   'ego_label_dict': ego_label_torch_dict,
                                   'ego_object_ids': ego_object_ids[0],
                                   'selected_cav_id_list':selected_cav_id_list,
